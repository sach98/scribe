...
//! - `scribe_keys`: Uses the scribe-keys library for more control
//!
...
// Note: Commands are accessed via shortcut::scribe_keys:: in lib.rs
...
pub fn init_shortcuts(app: &AppHandle) {
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::ScribeKeys => {
            if let Err(e) = scribe_keys::init_shortcuts(app) {
                error!("Failed to initialize scribe-keys shortcuts: {}", e);
                ...
                // Update settings to persist the fallback so we don't retry ScribeKeys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::ScribeKeys => scribe_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::ScribeKeys => scribe_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::ScribeKeys => scribe_keys::register_shortcut(app, binding),
    }
}
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::ScribeKeys => scribe_keys::unregister_shortcut(app, binding),
    }
}
...
pub fn change_keyboard_implementation_setting(
    ...
    // Initialize new implementation if needed (ScribeKeys needs state)
    if new_impl == KeyboardImplementation::ScribeKeys && initialize_scribe_keys_with_rollback(&app)? {
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::ScribeKeys => "scribe_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::ScribeKeys => scribe_keys::validate_shortcut(raw),
    }
}
...
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    match s {
        ...
        "scribe_keys" => KeyboardImplementation::ScribeKeys,
        other => {
            warn!(
...
/// Initialize ScribeKeys if not already initialized, with rollback on failure
fn initialize_scribe_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app.try_state::<scribe_keys::ScribeKeysState>().is_some() {
        return Ok(false); // Already initialized, caller should continue
    }

    if let Err(e) = scribe_keys::init_shortcuts(app) {
        error!("Failed to initialize ScribeKeys: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize ScribeKeys: {}. Reverted to Tauri.",
            e
        ));
    }
