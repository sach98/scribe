//! Scribe-keys based keyboard shortcut implementation
//!
//! This module provides an alternative to Tauri's global-shortcut plugin
//! using the scribe-keys library for more control over keyboard events.
//!
//! ## Architecture
//!
...
/// State for the scribe-keys shortcut manager
pub struct ScribeKeysState {
...
impl ScribeKeysState {
    /// Create a new ScribeKeysState
    ...
    fn manager_thread(cmd_rx: Receiver<ManagerCommand>, app: AppHandle) {
        info!("scribe-keys manager thread started");
        ...
        loop {
            ...
            while let Some(event) = manager.try_recv() {
                if let Some((binding_id, hotkey_string)) = hotkey_to_binding.get(&event.id) {
                    debug!(
                        "scribe-keys event: binding={}, hotkey={}, state={:?}",
            ...
            match cmd_rx.recv_timeout(std::time::Duration::from_millis(10)) {
                Ok(cmd) => match cmd {
                    ...
                    ManagerCommand::Shutdown => {
                        info!("scribe-keys manager thread shutting down");
                        break;
                    }
                },
        ...
        info!("scribe-keys manager thread stopped");
    }
    ...
    fn do_register(
        ...
        debug!(
            "Registered scribe-keys shortcut: {} -> {:?}",
            binding_id, hotkey
        );
        Ok(())
    }
    ...
    fn do_unregister(
        ...
        if let Some(id) = binding_to_hotkey.remove(binding_id) {
            ...
            debug!("Unregistered scribe-keys shortcut: {}", binding_id);
        }
        Ok(())
    }
    ...
    pub fn start_recording(&self, app: &AppHandle, binding_id: String) -> Result<(), String> {
        ...
        debug!("Started scribe-keys recording mode");
        Ok(())
    }
    ...
    fn recording_loop(app: AppHandle, running: Arc<AtomicBool>) {
        while running.load(Ordering::SeqCst) {
            let event = {
                let state = match app.try_state::<ScribeKeysState>() {
                    Some(s) => s,
                    None => break,
                };
            ...
            if let Some(key_event) = event {
                ...
                // Emit to frontend
                if let Err(e) = app.emit("scribe-keys-event", &frontend_event) {
    ...
    pub fn stop_recording(&self) -> Result<(), String> {
        ...
        debug!("Stopped scribe-keys recording mode");
        Ok(())
    }
}

impl Drop for ScribeKeysState {
    fn drop(&mut self) {
...
/// Convert scribe-keys Modifiers to a list of strings
fn modifiers_to_strings(modifiers: scribe_keys::Modifiers) -> Vec<String> {
    let mut result = Vec::new();

    if modifiers.contains(scribe_keys::Modifiers::CTRL) {
        result.push("ctrl".to_string());
    }
    if modifiers.contains(scribe_keys::Modifiers::OPT) {
        #[cfg(target_os = "macos")]
        result.push("option".to_string());
        #[cfg(not(target_os = "macos"))]
        result.push("alt".to_string());
    }
    if modifiers.contains(scribe_keys::Modifiers::SHIFT) {
        result.push("shift".to_string());
    }
    if modifiers.contains(scribe_keys::Modifiers::CMD) {
        #[cfg(target_os = "macos")]
        result.push("command".to_string());
        #[cfg(not(target_os = "macos"))]
        result.push("super".to_string());
    }
    if modifiers.contains(scribe_keys::Modifiers::FN) {
...
/// Validate a shortcut string for the ScribeKeys implementation.
/// ScribeKeys is more permissive: allows modifier-only combos and the fn key.
pub fn validate_shortcut(raw: &str) -> Result<(), String> {
    if raw.trim().is_empty() {
        return Err("Shortcut cannot be empty".into());
    }
    // ScribeKeys accepts modifier-only, key-only, and modifier+key combos
    // Just verify the string is parseable
    raw.parse::<Hotkey>()
        .map(|_| ())
        .map_err(|e| format!("Invalid shortcut for ScribeKeys: {}", e))
}

/// Initialize scribe-keys shortcuts
pub fn init_shortcuts(app: &AppHandle) -> Result<(), String> {
    let state = ScribeKeysState::new(app.clone())?;
    ...
    for (id, default_binding) in default_bindings {
        ...
        if let Err(e) = state.register(&binding) {
            error!(
                "Failed to register scribe-keys shortcut {} during init: {}",
                id, e
            );
        }
    }
    ...
    app.manage(state);
    info!("scribe-keys shortcuts initialized");
    Ok(())
}
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Some(cancel_binding) = get_settings(&app_clone).bindings.get("cancel").cloned() {
                if let Some(state) = app_clone.try_state::<ScribeKeysState>() {
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    let state = app
        .try_state::<ScribeKeysState>()
        .ok_or("ScribeKeysState not initialized")?;
...
#[tauri::command]
#[specta::specta]
pub fn start_scribe_keys_recording(app: AppHandle, binding_id: String) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::ScribeKeys {
        return Err("scribe-keys is not the active keyboard implementation".into());
    }
...
#[tauri::command]
#[specta::specta]
pub fn stop_scribe_keys_recording(app: AppHandle) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::ScribeKeys {
        return Err("scribe-keys is not the active keyboard implementation".into());
    }
