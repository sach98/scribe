...
impl TranscriptionManager {
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        if std::env::var("SCRIBE_FORCE_TRANSCRIPTION_FAILURE").is_ok() {
            return Err(anyhow::anyhow!(
                "Simulated transcription failure (SCRIBE_FORCE_TRANSCRIPTION_FAILURE)"
            ));
        }
...
/// Map Scribe's whisper accelerator setting to a transcribe-cpp [`Backend`].
///
