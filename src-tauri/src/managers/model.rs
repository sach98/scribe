...
/// Where a model comes from and how Scribe obtains it — the routing discriminant
...
/// The base code Scribe matches a language *intent* on: a tag's primary subtag,
...
/// Bridges hf-hub's async download progress to Scribe's `model-download-progress`
...
impl ModelManager {
    ...
    /// models dropped in or downloaded outside Scribe show up without a restart.
    ...
    fn discover_custom_transcribe_models(
        ...
        for entry in fs::read_dir(models_dir)? {
            ...
            // Scribe-downloaded one. Legacy `.bin` files have no GGUF header, so
    ...
    /// shared Hugging Face cache, so models downloaded by Scribe (or any other
...
mod tests {
    ...
    fn test_discover_hf_cache_models_filters_by_arch() {
        ...
        let id = "scribe-computer/whisper-test/whisper-q8.gguf";
        ...
        assert!(
            ...
            if repo_id == "scribe-computer/whisper-test" && revision == "main")
        );
        assert_eq!(
